...
exports.parseAlbumUrls = function (artistUrl) {
  return (async () => {
    ...
    $('a').each((_, element) => {
      ...
      if (href && /(track|album)\/(.+)$/.exec(href)) {
        const normalizedHref = href.startsWith('/') ? href : `/${href}`;
        const fullUrl = new urlHelper.URL(normalizedHref, artistUrl).toString();

        // Avoid duplicates
        if (!albumLinks.some(item => item.url === fullUrl)) {
          albumLinks.push({ url: fullUrl, title });
        }
      }
    });
